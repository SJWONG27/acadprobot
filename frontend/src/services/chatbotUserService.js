...
const SPRING_API = "/chatbot/user";

export const joinChatbot = async(user_id, refercode)=>{
    const response = await springapi.post(`${SPRING_API}/`, {user_id, refercode});
...
export const leaveChatbot = async(user_id, chatbot_id)=>{
    const response = await springapi.delete(`${SPRING_API}/`, {user_id, chatbot_id});
    return response.data;
}
